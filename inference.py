...
def load_model(model_size, cuda=None):
	...
	if model_size == 'L':
		model = SODAWideNetPlusPlus(3, 1, use_contour = True, deep_supervision = True, factorw = 4, dilation_rates = [[6, 10, 14, 18, 22], [6, 10, 14, 18]])
		model.to(cuda)
		checkpoint = torch.load('checkpoints/DUTSSODAWideNet++L.pt', map_location=cuda)

	elif model_size == 'M':
		model = SODAWideNetPlusPlus(3, 1, use_contour = True, deep_supervision = True, factorw = 2, dilation_rates = [[6, 10, 14, 18, 22], [6, 10, 14, 18]])
		model.to(cuda)
		checkpoint = torch.load('checkpoints/DUTSSODAWideNet++M.pt', map_location=cuda)
	
	elif model_size == 'S':
		model = SODAWideNetPlusPlus(3, 1, use_contour = True, deep_supervision = True, factorw = 1, dilation_rates = [[6, 10, 14, 18, 22], [6, 10, 14, 18]])
		model.to(cuda)
