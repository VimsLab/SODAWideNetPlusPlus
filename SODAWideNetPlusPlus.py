...
class SODAWideNetPlusPlusMC(nn.Module):
	...
	dilation_rates = [[1, 2, 3, 4, 5], [1, 2, 3, 4]]):
		...
		super(SODAWideNetPlusPlusMC, self).__init__()
		self.n_channels = n_channels
		self.n_classes = n_classes
		self.bilinear = bilinear
		self.ssl = ssl
		...
		if self.contour:
			...
			self.mrffamcontour2 = OutConv(64 * factorw, n_classes)
			self.mrffamcontour3 = OutConv((128 * factorw) // factor, n_classes)

			self.outlpmcontour2 = OutConv((128 * factorw) // factor, n_classes)
			self.outlpmcontour1 = OutConv(64 * factorw, n_classes)

			self.outlcontour3 = OutConv((128 * factorw) // factor, n_classes)
			self.outlcontour2 = OutConv(64 * factorw, n_classes)
	...
	def forward(self, x):
		...
		if self.contour:
			...
			ll_smallest_contour = self.up3b(self.mrffamcontour2(ds1))
			contours.append(ll_smallest_contour)

			ll_small_contour = self.up2b(self.mrffamcontour3(ds2))
			contours.append(ll_small_contour)

			lpmcontour1_ = self.up3b(self.outlpmcontour1(res1))
			contours.append(lpmcontour1_)

			lpmcontour2_ = self.up2b(self.outlpmcontour2(res2))
			contours.append(lpmcontour2_)

			lcontour2_ = self.up3b(self.outlcontour2(x2))
			contours.append(lcontour2_)

			lcontour3_ = self.up2b(self.outlcontour3(x3))
			contours.append(lcontour3_)
...
class SODAWideNetPlusPlusEncoder(nn.Module):
	...
	bilinear = True,
	factorw = 1,
	dilation_rates = [[6, 10, 14, 18, 22], [6, 10, 14, 18]]):

		super(SODAWideNetPlusPlusEncoder, self).__init__()
		self.n_channels = n_channels
		self.n_classes = n_classes
		self.bilinear = bilinear
		self.inc = nn.Sequential(
		...
		self.linear = nn.Linear((128 * factorw) // factor, n_classes)

		self.batchnorm = nn.BatchNorm1d(n_classes)
	...
	def forward(self, x):
		...
		b, _, _, _ = x3.size()
		gap_features = self.gap(x3)
		return self.batchnorm(self.linear(gap_features.view(b, -1)))
